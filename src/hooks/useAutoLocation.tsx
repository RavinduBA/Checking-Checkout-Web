...
export const useAutoLocation = () => {
  ...
  const [shouldShowLocationSelect, setShouldShowLocationSelect] = useState(true);
  ...
  const fetchLocationsAndAutoSelect = async () => {
    try {
      ...
      if (isAdmin) {
        ...
        userLocations = locations.filter(location => 
          permissions[location.name] && 
          Object.values(permissions[location.name]).some(permission => permission === true)
        );
      }
