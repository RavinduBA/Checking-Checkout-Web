...
import { Percent, Download, Calendar, Users, UserCheck } from "lucide-react";
...
export default function CommissionReports() {
  ...
  const calculateTotals = () => {
    const totals = filteredCommissions.reduce(
      ...
      { totalGuideCommission: 0, totalAgentCommission: 0, totalReservationValue: 0 }
    );
  ...
  const exportData = () => {
    const csvContent = [
      ['Reservation Number', 'Guest Name', 'Check-in', 'Check-out', 'Total Amount', 'Guide', 'Guide Commission', 'Agent', 'Agent Commission', 'Status'],
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              <Label htmlFor="selected_person">
                {filterType === "guide" ? "Guide" : filterType === "agent" ? "Agent" : "Person"}
              </Label>
              <Select 
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder={
                    filterType === "guide" ? "Select guide" : 
                    filterType === "agent" ? "Select agent" : 
                    "Select person"
                  } />
                </SelectTrigger>
                <SelectContent>
                  {filterType === "guide" && guides.map((guide) => (
                    <SelectItem key={guide.id} value={guide.id}>
                      {guide.name}
                    </SelectItem>
                  ))}
                  {filterType === "agent" && agents.map((agent) => (
