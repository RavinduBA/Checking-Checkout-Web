	...
	const fetchIncomeBreakdown = async () => {
        ...
        `);
          ...
          `)
				...
				paymentsQuery = supabase.from("payments").select(`
            id, created_at, amount, currency, payment_type, notes,
            accounts(name),
            reservations(guest_name, reservation_number, location_id)
          `);
					...
					<CardContent className="p-4 sm:p-6">
						<div className="flex items-center justify-between">
							<div>
								...
								<p
									className={`tex-lg sm:text-3xl font-bold ${summary.netProfit >= 0 ? "text-blue-900" : "text-red-900"}`}
								>
							...
							<DollarSign
								className={`size-5 ${summary.netProfit >= 0 ? "text-blue-600" : "text-red-600"}`}
							/>
						</div>
					</CardContent>
				</Card>
