  ...
  const fetchAccountBalances = async () => {
    try {
      ...
      for (const account of accounts) {
        ...
        const [incomeResult, expenseResult, transfersFromResult, transfersToResult] = await Promise.all([
          incomeQuery,
          expenseQuery,
          transfersFromQuery,
          transfersToQuery
        ...
        const totalIncome = incomeData.reduce((sum, item) => sum + parseFloat(item.amount.toString()), 0);
        const totalExpenses = expenseData.reduce((sum, item) => sum + parseFloat(item.amount.toString()), 0);
        const transfersOut = transfersFromData.reduce((sum, item) => sum + parseFloat(item.amount.toString()), 0);
        const transfersIn = transfersToData.reduce((sum, item) => sum + parseFloat(item.amount.toString()) * parseFloat(item.conversion_rate.toString()), 0);

        const currentBalance = account.initial_balance + totalIncome - totalExpenses - transfersOut + transfersIn;
        const transactionCount = incomeData.length + expenseData.length + transfersFromData.length + transfersToData.length;
