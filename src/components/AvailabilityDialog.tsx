		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>
						The selected room is not available for the requested dates. Here are
						your options:
					</DialogDescription>
				</DialogHeader>
					...
					<Alert className="border-orange-200 bg-orange-50">
						...
						<AlertDescription className="text-orange-800">
							<div className="font-medium mb-2">
								{originalRoom?.room_number} - {originalRoom?.room_type} is not
								available
							</div>
							<div className="text-sm">
								Requested dates: {formatDate(checkInDate)} -{" "}
									...
									>
										<div>
											...
											<div className="text-sm text-muted-foreground">
												{formatDate(conflict.check_in_date)} -{" "}
												{formatDate(conflict.check_out_date)}
											</div>
										</div>
										<Badge
											className={cn("text-xs", getStatusColor(conflict.status))}
										>
											{conflict.status}
										</Badge>
									</div>
					...
					{sameLocationAlternatives.length === 0 &&
						otherLocationAlternatives.length === 0 && (
							<Alert>
								...
								<AlertDescription>
									No alternative rooms are available for the selected dates. You
									may need to choose different dates or contact the guest to
