...
export const AvailabilityCalendarPopover: React.FC<AvailabilityCalendarPopoverProps> = ({
  ...
  const loadMonthAvailability = useCallback(async (month: Date) => {
    ...
    for (let i = 1; i <= totalDays; i++) {
      const dayKey = format(new Date(month.getFullYear(), month.getMonth(), i), 'yyyy-MM-dd');
    ...
    try {
      ...
      for (let day = new Date(Math.max(startOfMonth.getTime(), today.getTime())); day <= endOfMonth; day.setDate(day.getDate() + 1)) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-fit p-0 min-w-[680px]" align="start" sideOffset={4}>
        {selectedRoomId ? (
          <div className="p-6">
            {/* Header with Selected Dates */}
            <div className="mb-6 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-lg">Select Reservation Dates</h3>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-muted/50 rounded-lg">
                <div className="text-center">
                  <div className="text-sm text-muted-foreground mb-1">Check-in Date</div>
                  <div className={cn(
                ...
                <div className="text-center">
                  <div className="text-sm text-muted-foreground mb-1">Check-out Date</div>
                  <div className={cn(
                    "text-base font-semibold",
                    checkOut ? "text-foreground" : "text-muted-foreground"
                  )}>
                    {checkOut ? format(checkOut, 'EEE, MMM dd, yyyy') : 'Select check-in first'}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="border rounded-lg p-3 bg-white">
                  <Calendar
                    ...
                    modifiersClassNames={{
                      booked: "bg-red-100 text-red-800 line-through cursor-not-allowed hover:bg-red-100",
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  {!checkIn && <span className="text-xs text-muted-foreground">(Select check-in first)</span>}
                </div>
                <div className={cn(
            ...
            {checkIn && checkOut && (
              <div className="mt-6 flex justify-between items-center">
                <div className="text-sm text-muted-foreground">
                  Duration: {Math.ceil((checkOut.getTime() - checkIn.getTime()) / (1000 * 60 * 60 * 24))} night(s)
                </div>
