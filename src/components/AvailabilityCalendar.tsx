...
export function AvailabilityCalendar({
  ...
  const [activeCalendar, setActiveCalendar] = React.useState<"checkin" | "checkout">("checkin")
  const [availabilityCache, setAvailabilityCache] = React.useState<Map<string, DayAvailability>>(new Map())
        ...
        for (let i = 0; i < uncachedDays.length; i += batchSize) {
          ...
          const promises = batch.map(async (day) => {
            ...
            try {
              ...
              console.error(`Error checking availability for ${dayKey}:`, error)
              return {
                dayKey,
                availability: {
                  isAvailable: false,
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="border rounded-lg p-3">
              {activeCalendar === "checkin" ? (
                <Calendar
                  ...
                  modifiers={{
                    available: (day) => {
                      if (isBefore(day, new Date()) || !selectedRoomId) return false
                      const dayKey = format(day, "yyyy-MM-dd")
                      const availability = availabilityCache.get(dayKey)
                      return availability ? availability.isAvailable : false
                    },
                  ...
                  modifiersClassNames={{
                    ...
                    unavailable: "bg-red-100 text-red-800 line-through cursor-not-allowed hover:bg-red-100",
                  }}
                />
              ) : (
                <Calendar
                  ...
                  modifiers={{
                    available: (day) => {
                      if (isBefore(day, minCheckOutDate!) || !selectedRoomId) return false
                      const dayKey = format(day, "yyyy-MM-dd")
                      const availability = availabilityCache.get(dayKey)
                      return availability ? availability.isAvailable : false
                    },
            ...
            {checkIn && checkOut && (
              <div className="bg-blue-50 p-3 rounded-md border border-blue-200">
                <div className="text-sm font-medium text-blue-800">Selected Dates:</div>
                ...
                <div className="text-sm font-medium text-blue-800 mt-1">
                  Total nights: {Math.ceil((checkOut.getTime() - checkIn.getTime()) / (1000 * 60 * 60 * 24))}
                </div>
              </div>
            )}
