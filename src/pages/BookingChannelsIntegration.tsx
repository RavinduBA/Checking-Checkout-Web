...
export default function BookingChannelsIntegration() {
	...
	const calculateStats = (bookings: ExternalBooking[]): SyncStats => {
		...
		bookings.forEach((booking) => {
			// Source breakdown
			sourceBreakdown[booking.source] =
				(sourceBreakdown[booking.source] || 0) + 1;
	...
	const exchangeInviteCode = async (inviteCode: string) => {
		...
		try {
			...
			if (data && data.success) {
				toast.success(
					"Token exchange successful! You can now sync with booking channels.",
				);
				toast.info(
					"Please save your refresh token in Supabase secrets as CHANNEL_REFRESH_TOKEN",
				);
	...
	const getLocationFromBooking = (
		...
		if (booking.room_name && booking.room_name.includes("Room 609309")) {
			const rustyLocation = locations.find((loc) =>
				loc.name.toLowerCase().includes("rusty"),
			);
			if (rustyLocation) {
				console.log(
					`Forcing booking ${booking.external_id} with Room 609309 to ${rustyLocation.name}`,
				);
		...
		console.log(
			"Raw data keys:",
			booking.raw_data ? Object.keys(booking.raw_data) : "no raw_data",
		);
				...
				<Card>
					...
					<CardContent>
						<div className="text-lg sm:text-2xl font-bold">
							{syncStats.totalBookings}
						</div>
						<p className="text-xs text-muted-foreground">
							External bookings synced
						</p>
					</CardContent>
				</Card>
					...
					<CardContent>
						<div className="text-lg sm:text-2xl font-bold">
							{syncStats.recentSync
								? format(new Date(syncStats.recentSync), "MMM dd")
								: "Never"}
						</div>
						<p className="text-xs text-muted-foreground">
							{syncStats.recentSync
								? format(new Date(syncStats.recentSync), "HH:mm")
								: "No sync yet"}
						</p>
					</CardContent>
				</Card>
				...
				<TabsContent value="bookings" className="space-y-4">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Bookings synchronized from booking channels and other external
								sources
							</CardDescription>
						</CardHeader>
						<CardContent>
							...
							<div className="space-y-4">
								<div className="grid grid-cols-2 gap-4">
									...
									<div>
										...
										<div className="mt-1">
											<p className="font-medium">
												{selectedBooking.room_name || "Unknown"}
											</p>
											<p className="text-sm text-muted-foreground">
										...
										<div className="mt-1">
											...
											{getPaymentInfo(selectedBooking) && (
												<p className="text-sm text-green-600">
													Expected Payout:{" "}
													{getPaymentInfo(selectedBooking)?.payout} LKR
												</p>
											)}
										</div>
									</div>
									...
									<div>
										...
										<div className="bg-gray-50 p-3 rounded-lg space-y-1 text-sm">
											<div className="flex justify-between">
												...
												<span>
													{getPaymentInfo(selectedBooking)?.basePrice} LKR
												</span>
											</div>
											<div className="flex justify-between text-red-600">
												<span>Host Fee:</span>
												<span>
													-{getPaymentInfo(selectedBooking)?.hostFee} LKR
												</span>
											</div>
									...
									<div className="bg-gray-50 p-3 rounded-lg space-y-1 text-sm">
										...
										{selectedBooking.last_synced_at && (
											<div className="flex justify-between">
												...
												<span>
													{format(
														new Date(selectedBooking.last_synced_at),
														"MMM dd, yyyy HH:mm",
													)}
												</span>
											</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									...
									<div className="space-y-2">
										{Object.entries(syncStats.sourceBreakdown).map(
											([source, count]) => (
												<div
													key={source}
								...
								<div>
									...
									<div className="space-y-2">
										{Object.entries(syncStats.statusBreakdown).map(
											([status, count]) => (
												<div
													key={status}
													className="flex items-center justify-between"
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										Map your booking channel properties to internal locations
										and rooms
									</CardDescription>
								</div>
							</div>
							...
							<div className="space-y-4">
								{locations.map((location) => {
									const mapping = propertyMappings.find(
										(m) => m.locationName === location.name,
									);
									return (
										<Card
											key={location.id}
											...
											<CardHeader className="pb-3">
												<div className="flex items-center justify-between">
													...
													<Badge variant={mapping ? "default" : "secondary"}>
														{mapping
															? `${mapping.channelProperties.length} Properties`
															: "Not Mapped"}
													</Badge>
												</div>
											</CardHeader>
												...
												{mapping && (
													<div className="space-y-2">
														<Label className="text-sm font-medium">
															Mapped Channel Properties:
														</Label>
														<div className="flex flex-wrap gap-2">
															{mapping.channelProperties.map(
																(property, index) => (
																	<Badge
																		key={index}
																		variant="outline"
												...
												<div className="flex items-start gap-3">
													...
													<div className="space-y-2">
														...
														<ol className="text-sm text-amber-700 space-y-1 ml-4 list-decimal">
															...
															<li>
																Generate an invite code for this application
															</li>
															<li>
																Enter the invite code below to complete setup
															</li>
														</ol>
													</div>
												</div>
